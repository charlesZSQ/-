...
word SPI_1_InpLen;                     /* Length of input buffer's content */
static SPI_1_TComData *InpPtrR;        /* Pointer for reading from input buffer */
static SPI_1_TComData *InpPtrW;        /* Pointer for writing to input buffer */
static SPI_1_TComData InpBuffer[SPI_1_INP_BUF_SIZE]; /* Input buffer SPI commmunication */
word SPI_1_OutLen;                     /* Length of output bufer's content */
static SPI_1_TComData *OutPtrR;        /* Pointer for reading from output buffer */
static SPI_1_TComData *OutPtrW;        /* Pointer for writing to output buffer */
static SPI_1_TComData OutBuffer[SPI_1_OUT_BUF_SIZE]; /* Output buffer for SPI commmunication */
...
byte SPI_1_RecvChar(SPI_1_TComData *Chr)
{
  ...
  if (SPI_1_InpLen > 0U) {             /* Is number of received chars greater than 0? */
    EnterCritical();                   /* Enter the critical section */
    SPI_1_InpLen--;                    /* Decrease number of received chars */
    *Chr = *(InpPtrR++);               /* Read the char */
    /*lint -save  -e946 Disable MISRA rule (17.2) checking. */
    if (InpPtrR >= (InpBuffer + SPI_1_INP_BUF_SIZE)) { /* Is the pointer out of the receive buffer? */
...
{
  if (SPI_1_OutLen == SPI_1_OUT_BUF_SIZE) { /* Is number of chars in buffer the same as the size of transmit buffer? */
    return ERR_TXFULL;                 /* If yes then error */
  }
  EnterCritical();                     /* Enter the critical section */
  SPI_1_OutLen++;                      /* Increase number of bytes in the transmit buffer */
  *(OutPtrW++) = Chr;                  /* Store char to buffer */
  /*lint -save  -e946 Disable MISRA rule (17.2) checking. */
  if (OutPtrW >= (OutBuffer + SPI_1_OUT_BUF_SIZE)) { /* Is the pointer out of the transmit buffer? */
  ...
  if((SerFlag & RUNINT_FROM_TX) == 0U) { /* Transmition running? */
    ...
    SPI1DRL = *(OutPtrR);              /* Store char to transmitter register and start transmition */
...
byte SPI_1_SendBlock(const SPI_1_TComData *Ptr,word Size,word *Snd)
{
  ...
  while((count < Size) && (SPI_1_OutLen < SPI_1_OUT_BUF_SIZE)) { /* While there is some char desired to send left and output buffer is not full do */
    EnterCritical();                   /* Enter the critical section */
  ...
  /*lint -restore Enable MISRA rule (17.2) checking. */
    ...
    if ((count == Size) || (SPI_1_OutLen == SPI_1_OUT_BUF_SIZE)) { /* Is the last desired char put into buffer or the buffer is full? */
...
{
  EnterCritical();                     /* Enter the critical section */
  SPI_1_InpLen = 0U;                   /* Set number of chars in the receive buffer to 0 */
...
{
  EnterCritical();                     /* Enter the critical section */
  SPI_1_OutLen = 0U;                   /* Set number of chars in the transmit buffer to 0 */
...
*/
/*
word SPI_1_GetCharsInRxBuf(void)
...
*/
/*
word SPI_1_GetCharsInTxBuf(void)
...
ISR(SPI_1_Interrupt)
{
  SPI_1_TComData Data = 0U;            /* Temporary variable for data */
  ...
  Data = SPI1DRL;                      /* Read data from receiver */
  if(SPI_1_InpLen < SPI_1_INP_BUF_SIZE) { /* Is number of bytes in the receive buffer lower than size of buffer? */
    SPI_1_InpLen++;                    /* Increse number of chars in the receive buffer */
    *(InpPtrW++) = Data;               /* Save received char to the receive buffer */
    /*lint -save  -e946 Disable MISRA rule (17.2) checking. */
    if(InpPtrW >= (InpBuffer + SPI_1_INP_BUF_SIZE)) { /* Is the pointer out of the receive buffer? */
      InpPtrW = InpBuffer;             /* Set pointer on the first item into the receive buffer */
    }
    /*lint -restore Enable MISRA rule (17.2) checking. */
    if(SPI_1_InpLen == SPI_1_INP_BUF_SIZE) { /* Is number of bytes in the receive buffer equal to the size of buffer? */
  ...
  SPI_1_OutLen--;                      /* Decrease number of chars in the transmit buffer */
  OutPtrR++;                           /* Increment pointer */
  /*lint -save  -e946 Disable MISRA rule (17.2) checking. */
  if (OutPtrR >= (OutBuffer + SPI_1_OUT_BUF_SIZE)) { /* Is the pointer out of the transmit buffer? */
    OutPtrR = OutBuffer;               /* Set pointer on the first item into the transmit buffer */
  }
  /*lint -restore Enable MISRA rule (17.2) checking. */
  if (SPI_1_OutLen) {                  /* Is number of bytes in the transmit buffer greather then 0? */
    SPI1DRL = *(OutPtrR);              /* Store char to transmitter register */
  } else {
  ...
  if(Flags & ON_ERROR) {               /* Is any error flag set? */
    __DI();
    SPI_1_OnError();                   /* If yes then invoke user event */
  } else {
    if(Flags & ON_FULL_RX) {           /* Is OnFullRxBuf flag set? */
      __DI();
      SPI_1_OnFullRxBuf();             /* If yes then invoke user event */
    }
  }
  if (Flags & ON_FREE_TX) {            /* Is flag "OnFreeTxBuf" set? */
    __DI();
    SPI_1_OnFreeTxBuf();               /* If yes then invoke user event */
  }
}
...
{
  /* SPI1CR1: SPIE=0,SPE=0,SPTIE=0,MSTR=0,CPOL=0,CPHA=1,SSOE=0,LSBFE=0 */
  SPI1CR1 = 0x04U;                     /* Reset the device register */
  (void)SPI1SR;                        /* Read the status register */
  (void)SPI1DRL;                       /* Read the device register */
  /* SPI1BR: ??=0,SPPR2=0,SPPR1=1,SPPR0=0,??=0,SPR2=1,SPR1=0,SPR0=0 */
  SPI1BR = 0x24U;                      /* Set the baud rate register */
  /* SPI1CR2: ??=0,XFRW=0,??=0,MODFEN=0,BIDIROE=0,??=0,SPISWAI=0,SPC0=0 */
  SPI1CR2 = 0x00U;                     /* Set control register 2 */
  /* SPI1CR1: SPIE=1,SPE=1,SPTIE=0,MSTR=1,CPOL=1,CPHA=1,SSOE=0,LSBFE=0 */
  SPI1CR1 = 0xDCU;                     /* Set control register 1 */
  ...
  SPI_1_InpLen = 0U;                   /* No char in the receive buffer */
  InpPtrR = InpBuffer;                 /* Set read pointer on the first item in the receive buffer */
  InpPtrW = InpBuffer;                 /* Set write pointer on the first item in the receive buffer */
  SPI_1_OutLen = 0U;                   /* No char in the transmit buffer */
  OutPtrR = OutBuffer;                 /* Set read pointer on the first item in the transmit buffer */
  OutPtrW = OutBuffer;                 /* Set write pointer on the first item in the transmit buffer */
}
